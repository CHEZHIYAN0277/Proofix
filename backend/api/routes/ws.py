...
_TERMINAL_LIFECYCLE = frozenset({"run.completed", "run.failed"})
...
async def ws_run_timeline(websocket: WebSocket, run_id: str) -> None:
    ...
    # Lifecycle frames replay after the agent timeline so a client attaching to
    # a finished run sees the ending last, in the order it happened.
    ...
    try:
        while True:
            ...
            if current and current.status in _TERMINAL_STATUSES:
                ...
                await websocket.send_json({"type": "run.completed", "status": current.status})
                break
